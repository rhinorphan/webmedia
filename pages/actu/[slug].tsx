  ...
  const styles = {
    block: {
      blockquote: ({children} : any) => {
        return (
          <blockquote className='border-l-4 border-cyan-400 px-4 text-3xl font-baskerville bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent'>
            {children}
          </blockquote>
        )
      },
      ...
      <article className='px-10 md:px-0 md:max-w-3xl mx-auto'>
        ...
        <div className='flex items-center gap-2 pt-4'>
          ...
          <p className='font-extralight text-sm font-franklin'>
            Blog post by <span className='bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent font-semibold'>{post.author.name}</span> - Published at {" "} {new Date(post.publishedAt).toLocaleString()}
          </p>
        </div>
