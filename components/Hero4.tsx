    ...
    <div className='flex flex-row gap-6 md:justify-around'>
      {props.map(prop => (
          <Link key={prop._id} href={`/culture/${prop.slug.current}`}>
            <div className='group border-poppy border-[3px] rounded-2xl overflow-hidden md:overflow-visible cursor-pointer card min-w-[350px] md:border-0 md:max-w-lg md:items-center bg-[#000000] md:bg-opacity-0 shadow-2xl md:shadow-none z-50'>
              <div className='overflow-hidden  md:overflow-visible rounded-xl'>
                <img src={
                  urlFor(prop.mainImage).url()!
                } alt="" className='w-full h-56 md:h-96 min-h-[225px] object-cover group-hover:scale-105 md:rounded-2xl md:border-poppy md:border-[3px]'/>
